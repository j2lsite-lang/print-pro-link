...
const CITY_ANGLES: ((c: CityData) => string)[] = [
  ...
  (c) =>
    `Les professionnels ${de(c.name)} et du secteur de ${c.department} commandent leur impression en quelques clics : configuration en ligne, validation du fichier, puis livraison à ${c.name}.`,
  (c) =>
    `Commerçants, artisans et entreprises ${de(c.name)} disposent avec J2L Print d'une imprimerie en ligne complète : large catalogue, prix transparents et livraison ${article(c.department).dans}.`,
  (c) =>
    `À ${c.name}, gagnez du temps sur vos impressions professionnelles : tout se configure en ligne et vos supports sont livrés sur place, ${article(c.region).dans}.`,
];
...
const CITY_USE_BLOCKS: ((c: CityData) => ContentSection)[] = [
  ...
  (c) => ({
    ...
    paragraphs: [
      `Pas besoin de boutique physique : vous configurez vos produits en ligne, profitez de tarifs dégressifs selon la quantité, et recevez votre commande à ${c.name}. C'est la solution adoptée par de nombreuses TPE, PME et associations ${article(c.department).de}.`,
    ],
  }),
];
...
export function cityIntro(c: CityData): string[] {
  ...
  return [
    CITY_ANGLES[idx](c),
    `J2L Print n'est pas une imprimerie physique à ${c.name} : c'est un service d'impression en ligne avec livraison rapide ${article(c.region).dans}, pensé pour les professionnels qui veulent commander à distance sans renoncer à la qualité.`,
  ];
}
...
export function citySections(c: CityData): ContentSection[] {
  ...
  return [
    ...
    {
      heading: `Livraison à ${c.name} et ${article(c.department).dans}`,
      paragraphs: [
  ...
  return [
    {
      ...
      a: `Non. J2L Print est une imprimerie en ligne : vous commandez à distance et nous livrons vos supports à ${c.name} et ${article(c.department).dans}.`,
    },
    {
